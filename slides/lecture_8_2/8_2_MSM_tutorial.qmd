...
We assume that the unobserved portion of the income $\epsilon_i$ is orthogonal to the wage rate $w_i$: $E[\epsilon_i w_i] = 0$
...
- To make the estimation simpler, assume that we know that $\sigma^2 = 1$
...
1. Calculate parameters $\gamma$ and $\tau$ from data $(c_i, l_i, w_i)_{i=1}^N$
...
m(\gamma, \tau) =
    \begin{bmatrix}
        \frac{1}{N}\sum_{i=1}^N \left(\hat{C}(w_i, \epsilon_i, \gamma, \tau) - c_i\right) \\
        \frac{1}{N}\sum_{i=1}^N \left(\hat{L}(w_i, \epsilon_i, \gamma, \tau) - l_i\right)
    \end{bmatrix} = 0
\end{equation}
$$
...
1. Declare the model as before, but now declare `e` as a model parameter 
2. Draw R observations from N(0.0, 1.0) and store in `es`
3. Create a `for` loop over R observations
    - Set $\epsilon$: `set_parameter_value(e, es[i])`
...
The MSM estimator needs to take into account that $\gamma, \tau \in [0, 1]$. 

- This is a typical **box constrained optimization**, where the limits of each variable are constants. 
- `Optim.optimize` supports this with solver [Fminbox](https://julianlsolvers.github.io/Optim.jl/stable/user/minimization/#Box-Constrained-Optimization), which sets up an Interior Point problem for box constraints.
  - However, this algorithm is not very efficient because Interior Point Optimization requires solving a sequence of problems with different barrier function coefficients...
  - We will use another trick

## Extra: transforming a constrained optimization

We can still use an unconstrained optimization algorithm if we apply a clever transformation

$$
\theta = \frac{\exp(z)}{1 + \exp(z)}
$$

which can take any $z \in (-\infty, \infty)$ and map into $\theta \in (0, 1)$

- Recall that, if $\theta^* = \arg \min_\theta F(\theta)$, then $\theta^*$ is also the minimizer of a positive monitonic transformation $G$ of $F$. This is, $\theta^* = \arg \min_\theta G(F(\theta))$


## Estimation: initial guess

We also need to be careful to set the initial guess $z_0$ rather then $\theta_0$
...
## Estimation: modified objective function
...
## Estimation: modified unconstrained optimization

```julia
using Optim
@time res = Optim.optimize(Q, z_0, Newton(), Optim.Options(x_tol = 1e-2));
```

- Note the huge tolerance used in this example, so we have low-quality estimates.
  - Still, I had to run this optimization outside the slide rendering engine because it takes over 10 minutes...
...
## Calculating $E[\frac{\partial c}{\partial \tau}]$
...
This is pretty close to the true analytical value: $\bar{\Psi} = -\gamma \bar{w_i}$
...
- But remember: not all models yield closed-form expressions! That's why MSM (and MSL) are quite powerful methods when applied correctly

## Final words

We did not estimate the standard errors of parameters nor the variance of $\bar{\psi}$. But you can already anticipate that it could take a lot of time!
...
- With bootstrapping, it would REALLY, **REALLY** take a lot of time
   - It took about 10 minutes to solve with few MC draws, only 100 observations, and huge tolerance. With a large data set, this could easily take hours or days to solve on my laptop!
   - Multiply that by 10,000 bootstrap draws and you quickly realize you'd need to optimize your code and rent an HPC if you want results before graduation.
