...
-   Unlike linear systems, nonlinear systems we don't have guarantees of closed-form solutions, so solving them is harder
...
Function iteration is guaranteed to converge to a fixed point $x^*$ if 1. $g$ is differentiable, and 2. the initial guess is "sufficiently close" an $x^*$ at which $\|g^\prime (x^{*}) \| < 1$
