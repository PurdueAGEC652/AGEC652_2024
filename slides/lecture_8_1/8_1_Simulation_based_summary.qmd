...
$$
E[g(y_i, x_i, \theta)] = E[y_i - f(x_i, \theta)] = 0
$$

where $x_i$ are observed characteristics and $\theta$ is a vector of model parameters.

- I.e., under the true parameter $\theta$, the observed optimal choices on average match the choices predicted by our model
  - This is analogous to a zero-mean residual condition
...
$$
E[g(y_i, x_i, \theta)] = E\left[E_\zeta[y_i - f(x_i, \theta, \zeta_i)]\right] = E\left[y_i - \int_{\zeta}  f(x_i, \theta, \zeta_i) f_\zeta(z)dz \right]
$$
