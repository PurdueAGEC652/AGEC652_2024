...
- For simplicity, we will assume these are the only two products available in this market segments.
...
$$
\hat{c}_{k,j} = \gamma_0 + \gamma_1 B^S_{k,j} + \gamma_2 t_{k,j} + \gamma_3 l_{k,j} + \gamma_4 \xi^S_k +\epsilon^S_{k,j}
$$
...
- Key idea: with fixed product characteristics, higher costs lead to higher prices without affecting demand^[In the script to generate this data, products have an unobserved quality index that affects both utility and costs.]
...
$$
\hat{c}_{k,j} = \gamma_0 + \gamma_1 B^S_{k,j} + \gamma_2 t_{k,j} + \gamma_3 l_{k,j} + \epsilon^S_{k,j} = \theta^S X^S_{k,j} + \epsilon^S_{k,j}
$$
...
With function `g_i(theta)`, we can use `Optim` package to minimize objective function `Q(theta)` 
